...
public class JoyconManager : MonoBehaviour
{
    ...
    private const ushort product_l1 = 0x2006;
    private const ushort product_r1= 0x2007;
    ...
    void Awake()
    {
        ...
        while (ptr != IntPtr.Zero)
        {
            ...
            if (enumerate.product_id == product_l1 || enumerate.product_id == product_r1 )
            {
                if (enumerate.product_id == product_l1)
                {
                    isLeft = true;
                    Debug.Log("Left Joy-Con connected.");
                }
                else if (enumerate.product_id == product_r1)
                {
                    isLeft = false;
